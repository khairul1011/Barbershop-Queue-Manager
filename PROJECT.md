...
- ~~Sistem pembayaran/invoicing.~~ **Update:** direvisi — DP 50% via QRIS (Xendit) sudah live, lihat [Bagian 2 §✅ Sudah Diselesaikan](#-sudah-diselesaikan). Bukan invoicing penuh, cuma gerbang pembayaran di depan alur booking WA untuk cegah no-show.
...
**Status: terverifikasi end-to-end** — satu booking WA asli dites penuh: chat booking → QR QRIS muncul di WA → pembayaran disimulasikan lewat Xendit Test Mode (`POST /v3/payment_requests/{id}/simulate`) → webhook `payment.succeeded` diterima server → baris `whatsapp_requests` ke-update `payment_status: 'paid'` → customer dapet notifikasi WA konfirmasi pembayaran, semua otomatis tanpa campur tangan manual. Token salah ke webhook ditolak 401, replay webhook yang sama nggak diproses dobel (idempotency lewat cek `payment_status === 'paid'`). Env var asli (`XENDIT_SECRET_KEY`, `XENDIT_CALLBACK_TOKEN`, `WEBHOOK_PORT`) sudah terpasang di `server/.env` VPS, kode sudah live di produksi.

---

## ✅ Sudah Diselesaikan (lainnya)
